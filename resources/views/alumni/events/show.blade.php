    ...
    <div class="bg-white rounded-2xl shadow-md overflow-hidden">
        ...
        <div class="p-6">
            ...
            <div class="mt-8 pt-4 border-t border-gray-200 flex justify-between items-center">
                <div>
                    @if($isRegistered)
                        <form action="{{ route('alumni.events.cancel', $event->id) }}" method="POST">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="bg-red-500 hover:bg-red-600 text-white px-6 py-2 rounded-xl transition">Cancel Registration</button>
                        </form>
                    @else
                        <form action="{{ route('alumni.events.register', $event->id) }}" method="POST">
                            @csrf
                            <button type="submit" class="bg-[#f5a623] hover:bg-[#f4b84a] text-[#1a3c5e] px-6 py-2 rounded-xl font-semibold transition">Register for Event</button>
                        </form>
                    @endif
                </div>
                @if(auth()->id() == $event->user_id)
                <div class="flex gap-3">
                    <a href="{{ route('alumni.events.edit', $event->id) }}" class="px-5 py-2 bg-[#1a3c5e] hover:bg-[#2d5a7b] text-white rounded-xl transition flex items-center gap-2">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
                        </svg>
                        Edit Event
                    </a>
                    <form action="{{ route('alumni.events.destroy', $event->id) }}" method="POST" onsubmit="return confirm('Delete this event?')">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="px-5 py-2 bg-red-500 hover:bg-red-600 text-white rounded-xl transition">Delete</button>
                    </form>
                </div>
                @endif
            </div>
